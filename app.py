...
# Helper function for recommendation quota
def calculate_recommendation_quota(item):
...
def get_customer(customer_id):
    ...
    try:
        ...
        parameters = [{"name": "@customer_id", "value": int(customer_id)}]
        items = list(container.query_items(query=query, parameters=parameters, enable_cross_partition_query=True))

        # Add sentiment polarity and recommendation quota
        for item in items:
            ...
            item["recommendation_quota"] = calculate_recommendation_quota(item)
...
def predict_risk():
    ...
    try:
        ...
        if not all(isinstance(x, (int, float)) for x in [age, gender, sentiment_score]):
